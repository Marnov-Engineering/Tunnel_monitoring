...
const uint8_t dataPerNode[RECEIVER_COUNT] = {
  ...
  4, 4, 1, 1, 1, 14, 14, 14                   // 2 sensorUjung → 3 data per node
};


String data[74];        // buffer input baru
String lastData[74];    // buffer nilai terakhir yang valid
int zeroCount[74] = {0}; // penghitung 0 berturut-turut
...
bool useFilter(int idx) {
  if (idx >= 0 && idx <= 28) return true;
  if (idx == 36 || idx == 37 || idx == 51 || idx == 50 || idx == 64 || idx == 65) return true;
  return false;
}


String dataHeater[74];
String dataMAC[500];
