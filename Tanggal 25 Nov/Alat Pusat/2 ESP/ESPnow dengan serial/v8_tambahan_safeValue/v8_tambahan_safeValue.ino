    ...
    for (int j = 0; j < 62; j++) {
      ...
      if (useFilter(j)) {
        ...
        if (lastData[j].length() == 0 && val == "0") {
          // pertama kali dan langsung nol
          lastData[j] = "0";
          zeroCount[j] = 0;
        }
        else if (val == "0") {
          if (zeroCount[j] >= 2) {
            // kalau sudah 3 kali berturut-turut → kirim "0"
            val = "0";
            lastData[j] = "0";
            zeroCount[j] = 0; // reset counter
          } else {
            // masih di bawah 3 kali → pakai lastData
            val = lastData[j];
            zeroCount[j]++;
          }
        }
        else {
          // kalau data baru bukan 0 → update lastData
          lastData[j] = val;
          zeroCount[j] = 0;
        }
      } else {
        // index lain → langsung kirim apa adanya
        lastData[j] = val;
        zeroCount[j] = 0;
      }
