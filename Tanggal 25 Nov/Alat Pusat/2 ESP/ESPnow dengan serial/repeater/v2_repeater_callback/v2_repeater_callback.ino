...
static uint8_t Pusat[] = {0x1C, 0x69, 0x20, 0x96, 0xFC, 0x60};
//1c:69:20:96:fc:60
...
void kirimPesanKePusat(const String &message) {

  quickEspNow.send(Pusat, (uint8_t*)message.c_str(), message.length());
...
//   // ---- Ambil data string dari pengirim ----
//   String incoming = "";
//   for (int i = 0; i < len; i++) incoming += (char)dataRaw[i];

//   // ---- Convert MAC ke string ----
//   char macStr[18];
//   sprintf(macStr, "%02X:%02X:%02X:%02X:%02X:%02X",
//           address[0], address[1], address[2],
//           address[3], address[4], address[5]);

//   // ---- Parsing ke dalam array data[] ----
//   for (int node = 0, dataIndex = 0; node < RECEIVER_COUNT; node++) {
...
//       // ---- Setelah parsing selesai, kirim balik ----
//       sprintf(replyMsg, "%s,%s", macStr, incoming.c_str());  
//       kirimPesanKePusat(replyMsg);
...
void dataReceived(uint8_t* address, uint8_t* dataRaw, uint8_t len, signed int rssi, bool broadcast) {
  ...
  String incoming = "";
  for (int i = 0; i < len; i++) {
    incoming += (char)dataRaw[i];
  }

  // ---- Convert MAC pengirim ke string ----
  char macStr[18];
  sprintf(macStr, "%02X:%02X:%02X:%02X:%02X:%02X",
          address[0], address[1], address[2],
          address[3], address[4], address[5]);

  // ---- Gabungkan pesan untuk dikirim ulang ----
  // Format: MAC,DATA
  String sendBack = String(macStr) + "," + incoming;

  // ---- Kirim kembali ke pusat ----
  kirimPesanKePusat(sendBack.c_str());

  Serial.print("REPEATER FORWARD: ");
...
void loop() {
  ...
  if (elapsed >= 2500 && elapsed < 5000) {
    ...
    //   if (!quickEspNow.send(heater1, (uint8_t *)message.c_str(), message.length())) {
    //     Serial.println(">>>>>>>>>> Message sent to heater1");
    //   } else {
    //     Serial.println(">>>>>>>>>> Message not sent to heater1");
    //   }
    //   delay(50);

    //   if (!quickEspNow.send(heater2, (uint8_t *)message.c_str(), message.length())) {
    //     Serial.println(">>>>>>>>>> Message sent to heater2");
    //   } else {
    //     Serial.println(">>>>>>>>>> Message not sent to heater2");
    //   }
    //   delay(50);

    //   if (!quickEspNow.send(heater3, (uint8_t *)message.c_str(), message.length())) {
    //     Serial.println(">>>>>>>>>> Message sent to heater3");
    //   } else {
    //     Serial.println(">>>>>>>>>> Message not sent to heater3");
    //   }
    //   delay(50);
