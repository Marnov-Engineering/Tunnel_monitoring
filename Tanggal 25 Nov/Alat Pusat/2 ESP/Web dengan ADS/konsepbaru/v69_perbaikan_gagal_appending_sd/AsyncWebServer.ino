...
const char* vSendData() {
  static char data[200];
  snprintf(data, sizeof(data),
           "%.2f,%.2f,%.2f,%.2f,%d,%d,%d,%d,%.2f,%.2f,%.2f,%d,%d,%d,%d,%d,%d,%.5f,%.5f,%.5f,%d,%d,%d",
           Watt1, Watt2, Watt3, Watt4,
           kecepatan1, kecepatan2, kecepatan3, kecepatan4,
           out_pid1, out_pid2, out_pid3,
           Heater_st_1, Heater_st_2, Heater_st_3,
...
// --- Versi efisien tanpa String dan tanpa heap ---
const char* vSendDataMAC() {
  static char dataMAC[1000];  // buffer statis global, tidak dialokasikan ulang
  int len = snprintf(
    dataMAC, sizeof(dataMAC),
  ...
  // Pastikan buffer tidak overflow
  if (len >= sizeof(dataMAC)) {
    dataMAC[sizeof(dataMAC) - 1] = '\0';
  }

  return dataMAC; // tidak perlu String(), langsung kembalikan pointer
}

const char* vSendDataHeat() {
  // Buffer statik agar tidak hilang setelah fungsi selesai
  ...
  return data; // pointer ke buffer statik
}
...
void vAsyncWebServer() {
  ...
  server.on("/", HTTP_GET, [](AsyncWebServerRequest *request){ 
              ...
              if(sedangMengirim){
                ...
                request->send(LittleFS, "/user.html", "text/html"); 
              }
  ...
  // });
    ...
    server.on("/data", HTTP_GET, [](AsyncWebServerRequest *request){
      ...
      sprintf(json,
        ...
        safeValue(suhu1), safeValue(suhu2), safeValue(suhu3), safeValue(suhu4), safeValue(suhu5), safeValue(suhu6), safeValue(suhu7),
        safeValue(Dsuhu1), safeValue(Dsuhu2), safeValue(Dsuhu3),
        safeValue(SBME1), safeValue(SBME2), safeValue(SBME3),
        safeValue(Humid1), safeValue(Humid2), safeValue(Humid3), safeValue(Humid4), safeValue(Humid5), safeValue(Humid6), safeValue(Humid7),
        safeValue(UHumid1), safeValue(UHumid2),
        safeValue(kec1), safeValue(kec2), safeValue(kec3), safeValue(kec4), safeValue(kec5), safeValue(kec6), safeValue(kec7),
        safeValue(Usuhu1), safeValue(Usuhu2),
        safeValue(Ukec1), safeValue(Ukec2),
        safeValue(USDP1), safeValue(USDP2),
        safeValue(USDPSekitar1), safeValue(USDPSekitar2)
      );

      request->send(200, "application/json", json);
      Serial.print("Task async web berjalan di core = ");
      Serial.println(xPortGetCoreID());
      Serial.println("json untuk wbserver async");
      Serial.println(json);
    ...
    server.on("/dataDin", HTTP_GET, [](AsyncWebServerRequest *request){
      ...
      char json[256];  // pastikan buffer cukup besar, bisa diperbesar jika perlu
  ...
  server.on("/dataSet", HTTP_GET, [](AsyncWebServerRequest *request){
    if(sedangMengirim){
        counterTubrukandataset++;
      }
    request->send_P(200, "text/plain", vSendData());
  });

  server.on("/dataSetHeat", HTTP_GET, [](AsyncWebServerRequest *request){
    if(sedangMengirim){
        counterTubrukandatasetheat++;
      }
    request->send_P(200, "text/plain", vSendDataHeat());
  });

  server.on("/dataMAC", HTTP_GET, [](AsyncWebServerRequest *request){
    if(sedangMengirim){
        counterTubrukandatamac++;
      }
    request->send_P(200, "text/plain", vSendDataMAC());
  });
  ...
  server.on("/resetSD", HTTP_GET, [](AsyncWebServerRequest *request){
    ...
    if(xSemaphoreTake(sdCardMutex, pdMS_TO_TICKS(50000)) == pdTRUE){
      removeDirRecursive(SD, "/CSVSpread");
      removeDirRecursive(SD, "/CSVFire");
      createDir(SD, "/CSVSpread");
      createDir(SD, "/CSVFire");
      UrutanSpread = 1;
      UrutanFirebase = 1;
      UrutanLogging = 1;
      counterParsing = 0;
