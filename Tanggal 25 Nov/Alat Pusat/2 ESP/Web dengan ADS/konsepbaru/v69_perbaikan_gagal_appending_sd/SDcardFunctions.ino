...
void writeBufferTunnel(const char* path) {
  if(xSemaphoreTake(sdCardMutex, pdMS_TO_TICKS(50000)) == pdTRUE){
    ...
    sprintf(buffer,
          ...
          "8,%lu,nan,%s,%s,%s,%s,%s,nan,nan\n"
          "9,%lu,nan,%s,%s,%s,%s,%s,nan,nan\n"
          ...
          waktuUpdate, safeValue(Usuhu1).c_str(), safeValue(UHumid1).c_str(), safeValue(Ukec1).c_str(),  safeValue(USDP1).c_str(), safeValue(USDPSekitar1).c_str(),
          waktuUpdate, safeValue(Usuhu2).c_str(), safeValue(UHumid2).c_str(), safeValue(Ukec2).c_str(),  safeValue(USDP2).c_str(), safeValue(USDPSekitar1).c_str(),
          waktuUpdate,SuhuDS1,
          waktuUpdate,SuhuDS2,
          waktuUpdate,SuhuDS3,
          waktuUpdate,BME1,
