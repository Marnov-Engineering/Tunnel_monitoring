...
// void printAllMACs() { //// potensi bermasalah 1
...
void printAllMACs() {
  // 1 MAC = 17 karakter ("AA:BB:CC:DD:EE:FF")
  // + 1 koma antar MAC + 2 karakter (^ dan *)
  // jadi total kira-kira (15 * 18) + 2 = 272 karakter
  char buffer[300];
  int offset = 0;

  offset += sprintf(buffer + offset, "^");  // awalan

  const int macCount = 15;  // jumlah sensor

  for (int i = 0; i < macCount; i++) {
    offset += sprintf(buffer + offset, "%02X:%02X:%02X:%02X:%02X:%02X",
                      allMACs[i][0], allMACs[i][1], allMACs[i][2],
                      allMACs[i][3], allMACs[i][4], allMACs[i][5]);

    if (i < macCount - 1) {
      offset += sprintf(buffer + offset, ",");  // tambahkan koma kecuali terakhir
    }
  }

  sprintf(buffer + offset, "*");  // akhiran
  ...
  for(;;){
    if (!bOTAStart) {
      if(bPertamaKirim){
        ...
        if(PengecekanFileSpread){
          // String Payload = vPersiapanPayloadJson(SD,bufferPathSpread);
          // Serial.println("Payload");
          // Serial.println(Payload);
          // vPengirimanSpreadsheet(Payload);
        ...
        if(PengecekanFileFire){
          // PrepareSelesai = vBacaFileDariSD(bufferPathFire);
          // vPrintHasilPersiapanFirebase();
          // vSendFirebase(bufferPathFire, bufferNameFire);
