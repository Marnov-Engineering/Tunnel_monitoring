// perbaikan string dan concat pada bagian ambildata
//penambahan variabel sdp sekitar

// v70 perbaikan static json dan penambahan variabel untuk firebase (watt)
...
int counterTubrukandataheat, counterTubrukandata, counterTubrukandataset, counterTubrukandatasetheat, counterTubrukandatamac,counterTubrukanhome;
bool sedangMengirim;
...
double arrayJson[42][12];
String arraywaktu[12];
int counterParsing = 0; 
int BanyakDataPengiriman = 0;
unsigned long timeUpdate;
int ResetWifi = 0;
bool BerhasilMenulisFire = false;
bool BerhasilMenulisSpread = false;
...
char serverPathSpreadSheet[256] = "/macros/s/AKfycbwpj8h0vbdLbXxzxLvuwgybVbNgSE5WbAf48IXItcYMqr9CRJ2FPmDJKmaV_boVXWuqiQ/exec";
...
void vSetupEEPROM(){
  ...
  // preferences.putInt("UrutanSpread", UrutanSpread);
  // preferences.putInt("UrutanFirebase", UrutanFirebase);
  // preferences.putInt("UrutanLogging", UrutanLogging);

  UrutanSpread = preferences.getInt("UrutanSpread", 0 );
  UrutanFirebase = preferences.getInt("UrutanFirebase", 0 );
  UrutanLogging = preferences.getInt("UrutanLogging", 0);
...
const char* safeValue(const String& val) {
  static char bufs[8][32];   // 8 buffer melingkar
  ...
  if (tmp.length() == 0 || tmp == "nan" || tmp == "-1.00" || tmp == "-999.00") {
    return zeroStr;
  }

  idx = (idx + 1) % 8;  // geser ke buffer berikutnya
  tmp.toCharArray(bufs[idx], sizeof(bufs[idx]));
  return bufs[idx];
}
...
void parseSerialData(String input) {
  ...
  readkec1[indexMA] = atof(safeValue(data[1]));
  readkec2[indexMA] = atof(safeValue(data[4]));
  readkec3[indexMA] = atof(safeValue(data[7]));
  readkec4[indexMA] = atof(safeValue(data[10]));
  readkec5[indexMA] = atof(safeValue(data[13]));
  readkec6[indexMA] = atof(safeValue(data[16]));
  readkec7[indexMA] = atof(safeValue(data[19]));
  readUkec1[indexMA] = atof(safeValue(data[23]));
  readUkec2[indexMA] = atof(safeValue(data[27]));
  ...
  SuhuDS1         = atof(safeValue(data[36]));
  BME1            = atof(safeValue(data[37]));
  ...
  SuhuDS2         = atof(safeValue(data[50]));
  BME2            = atof(safeValue(data[51]));
  ...
  SuhuDS3         = atof(safeValue(data[64]));
  BME3            = atof(safeValue(data[65]));
...
void vPemindahanData(){
  ...
  readkec7[indexMA] = atof(safeValue(data[19]));
  readUkec1[indexMA] = atof(safeValue(data[23]));
  readUkec2[indexMA] = atof(safeValue(data[28]));
  ...
  Ukec1 = String(AverageUkec1);
  USDP1 = data[24];
  USDPSekitar1 = data[25];

  Usuhu2 = data[26];
  UHumid2 = data[27]; 
  Ukec2 = String(AverageUkec2);
  USDP2 = data[29];
  USDPSekitar2 = data[30];
  // 31 tidak dipakai
  // 32 tidak dipakai
  // 33 tidak dipakai

// ================= HEATER 1 =================
Heater_st_1      = data[34].toInt();
Fan_Power_1      = data[35].toInt();
Power_Tunnel_1   = data[36].toInt();
WCSstate1        = !data[37].toInt();
SuhuDS1          = atof(safeValue(data[38]));
BME1             = atof(safeValue(data[39]));
setpoint1        = data[40].toFloat();
kp1              = data[41].toFloat();
ki1              = data[42].toFloat();
kd1              = data[43].toFloat();
out_pid1         = data[44].toFloat();
CalM_DinD1       = data[45].toFloat();
CalB_DinD1       = data[46].toFloat();
Max_PID_Tunnel_1 = data[47].toInt();

// ================= HEATER 2 =================
Heater_st_2      = data[48].toInt();
Fan_Power_2      = data[49].toInt();
Power_Tunnel_2   = data[50].toInt();
WCSstate2        = !data[51].toInt();
SuhuDS2          = atof(safeValue(data[52]));
BME2             = atof(safeValue(data[53]));
setpoint2        = data[54].toFloat();
kp2              = data[55].toFloat();
ki2              = data[56].toFloat();
kd2              = data[57].toFloat();
out_pid2         = data[58].toFloat();
CalM_DinD2       = data[59].toFloat();
CalB_DinD2       = data[60].toFloat();
Max_PID_Tunnel_2 = data[61].toInt();

// ================= HEATER 3 =================
Heater_st_3      = data[62].toInt();
Fan_Power_3      = data[63].toInt();
Power_Tunnel_3   = data[64].toInt();
WCSstate3        = !data[65].toInt();
SuhuDS3          = atof(safeValue(data[66]));
BME3             = atof(safeValue(data[67]));
setpoint3        = data[68].toFloat();
kp3              = data[69].toFloat();
ki3              = data[70].toFloat();
kd3              = data[71].toFloat();
out_pid3         = data[72].toFloat();
CalM_DinD3       = data[73].toFloat();
CalB_DinD3       = data[74].toFloat();
Max_PID_Tunnel_3 = data[75].toInt();
...
String ambilData(String dataTMP, char pemisah, int urutan) {
  stringData = 0;
  String perBagianData = "";
  char databagian[20];
  int j = 0;
  for (int i = 0; i < dataTMP.length(); i++){
    if (dataTMP[i] == pemisah){
      stringData++;
    }
    else if (stringData == urutan){
      //perBagianData.concat(data[i]);
      // perBagianData = String(data[i])
      
      if (j < sizeof(databagian) - 1) { 
        databagian[j++] = dataTMP[i];
      }
    }
    else if (stringData > urutan){
      databagian[j++] = '\0';
      perBagianData = String(databagian);
      break;
    }
  }
...
void vAmbilDataDariSerial2(){
  if (Serial2.available()>0) {
    ...
    sprintf(bufferEpochName, "/CSVFire/%d.csv", UrutanLogging);
    ...
    if (counterParsing >= 12){
      ...
      sprintf(bufferEpochName, "/CSVSpread/%d.txt", UrutanLogging);
      writeBufferSpreadSheet(bufferEpochName);

      timeUpdate = rtc.getEpoch();
      UrutanLogging++;
      if(UrutanLogging >= 1440){
        UrutanLogging = 0;
      }
      preferences.putInt("UrutanLogging", UrutanLogging);
      vTulisDataBaruFireEpoch(UrutanLogging);
      vTulisDataBaruSpreadEpoch(UrutanLogging);
      if (!BerhasilMenulisSpread && !BerhasilMenulisSpread){
        UrutanLogging--;
        preferences.putInt("UrutanLogging", UrutanLogging);
        vSetupSDcard();
      }
...
void vPrintHasilPersiapanFirebase(){
...
void TaskSendData( void * pvParameters ){
  ...
  TickType_t xFrequency = 60000/ portTICK_PERIOD_MS;
  ...
  for(;;){
    if (!bOTAStart) {
      if(bPertamaKirim){
        ...
        char bufferPathSpread[64];
        char bufferPathFire[64];
        char bufferNameFire[64];

        const char *dirPathSpread = "/CSVSpread";
        const char *dirPathFire = "/CSVFire";
        bool PengecekanFileFire = bPengechekanDuaFile(SD, dirPathFire);
        bool PengecekanFileSpread = bPengechekanDuaFile(SD, dirPathSpread);
        bool PrepareSelesai = false;
        // if(PengecekanFileSpread){
        //   String LocSpread = vSDEpochTerkecil(SD, dirPathSpread);
        //   // if (LocSpread != "") {
        //   //   snprintf(bufferPathSpread, sizeof(bufferPathSpread), "/CSVSpread/%s", LocSpread.c_str());
        //   //   Serial.printf("Path file terkecil untuk spreadsheet: %s\n", bufferPathSpread);
        //   // } 
        //   snprintf(bufferPathSpread, sizeof(bufferPathSpread), "/CSVSpread/%d.txt", UrutanSpread);
        //   Serial.printf("Path file untuk spreadsheet: %s\n", bufferPathSpread);

        // }else{
        //   Serial.println("Kurang dari 2 file — tidak dilakukan pemrosesan.");
        // }

        // if(PengecekanFileFire){
        //   String LocFire = vSDEpochTerkecil(SD, dirPathFire);
        //   // if (LocFire != "") {
        //     // snprintf(bufferPathFire, sizeof(bufferPathFire), "/CSVFire/%s", LocFire.c_str());
        //     // Serial.printf("Path file terkecil untuk firebase: %s\n", bufferPathFire);

        //     // snprintf(bufferNameFire, sizeof(bufferNameFire), "CSVFire/%s", LocFire.c_str());
        //     // Serial.printf("Nama file terkecil untuk firebase: %s\n", bufferNameFire);
        //   // } 
        //   snprintf(bufferPathFire, sizeof(bufferPathFire), "/CSVFire/%d.csv", UrutanFirebase);
        //   Serial.printf("Path file terkecil untuk firebase: %s\n", bufferPathFire);
        //   snprintf(bufferNameFire, sizeof(bufferNameFire), "CSVFire/%d.csv", UrutanFirebase);
        //   Serial.printf("Nama file terkecil untuk firebase: %s\n", bufferNameFire);
        ...
        // bool PengecekanFileFire = bPengechekanDuaFile(SD, dirPathFire);
        // bool PengecekanFileSpread = bPengechekanDuaFile(SD, dirPathSpread);
        unsigned long proctime = 1;
        int percobaankirim = 0;
        bool SpreadBerhasil = false;
        bool FireBerhasil = false;
        while ((PengecekanFileSpread || PengecekanFileFire) && ((60000 / proctime - 1) > percobaankirim) && (ResetWifi < 5)) {
          unsigned long startKirim = millis();
          if(PengecekanFileSpread){
            snprintf(bufferPathSpread, sizeof(bufferPathSpread), "/CSVSpread/%d.txt", UrutanSpread);
            Serial.printf("Path file untuk spreadsheet: %s\n", bufferPathSpread);
            bool ok = sendFileSpreadsheet(serverNameSpreadSheet, serverPort, serverPathSpreadSheet, bufferPathSpread);
            Serial.println(ok ? "Upload Firebase done" : "Upload firebase failed");
            if(!ok){
              ResetWifi++;
            }
            else{
          ...
          if(PengecekanFileFire){
            ...
            snprintf(bufferNameFire, sizeof(bufferNameFire), "CSVFire/%d.csv", UrutanFirebase);
            Serial.printf("Nama file terkecil untuk firebase: %s\n", bufferNameFire);
            bool ok = sendFileFirebase(serverName, serverPort, serverPath, bufferPathFire, bufferNameFire);
            Serial.println(ok ? "Upload Firebase done" : "Upload firebase failed");
            if(!ok){
              ResetWifi++;
          ...
          if(ResetWifi >= 5){
            WiFi.disconnect(false, false); // putus dan hapus konfigurasi
            Serial.println("Wifi telah di reset ......................................................... ");
          }else{
            unsigned long epochTime = getCurrentTime();
            rtc.setTime(epochTime);
            Serial.print("epochTime : ");
          ...
          if(FireBerhasil && SpreadBerhasil){
            ResetWifi = 0;
          }
          unsigned long finnishkirim = millis();
          proctime = finnishkirim - startKirim;
          if(proctime < 1000){
            proctime = 1000;
          }
          ...
          percobaankirim++;
          Serial.print("percobaankirim = ");
          Serial.println(percobaankirim);
          Serial.println("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++");
          PengecekanFileFire = bPengechekanDuaFile(SD, dirPathFire);
          PengecekanFileSpread = bPengechekanDuaFile(SD, dirPathSpread);
        }
        


        digitalWrite(LED_INDX, LOW);
        Serial.print("Akhir persiapan :");
        Serial.println(millis());
...
void loop() {
  // put your main code here, to run repeatedly:
    if(millis() - ADSMillis >= IntervalADSMillis){
      ...
      Serial.println("=== Counter Tubrukan ===");
      Serial.print("counterTubrukandataheat: ");     Serial.println(counterTubrukandataheat);
      Serial.print("counterTubrukandata: ");          Serial.println(counterTubrukandata);
      Serial.print("counterTubrukandataset: ");       Serial.println(counterTubrukandataset);
      Serial.print("counterTubrukandatasetheat: ");   Serial.println(counterTubrukandatasetheat);
      Serial.print("counterTubrukandatamac: ");       Serial.println(counterTubrukandatamac);
      Serial.print("counterTubrukanhome: ");          Serial.println(counterTubrukanhome);
      Serial.println("=========================");

      vReconnectWifi();
      Serial.print("UrutanSpread = ");
      Serial.println(UrutanSpread);
      Serial.print("UrutanFirebase = ");
      Serial.println(UrutanFirebase);
      Serial.print("UrutanLogging = ");
      Serial.println(UrutanLogging);

      Serial.print("Part = ");
      Serial.println(part);
      Serial.print("error count restarted = "); 
...
void vSendFirebase(const char* path, const char* name) {
  // Use this only for HTTPS
  // client.setInsecure();//skip verification because our server changes rootCA every month or so. Skip this as long as we get certificate we accept
  if (client.connect(serverName, serverPort)) {
    ...
    if(xSemaphoreTake(sdCardMutex, pdMS_TO_TICKS(50000)) == pdTRUE){
      ...
      CRC32File = vCheckCRCfile(SD, path, 10000);
      file.close();
      xSemaphoreGive(sdCardMutex);
    }
...
void vParsingResponseWebV2(const char* jsonResponse){
  ...
  // Parse JSON
  StaticJsonDocument<4096> StatcJson;
  DeserializationError error = deserializeJson(StatcJson, jsonResponse);
  if (error) {
  ...
  // Ambil data umum
  lastVersion = StatcJson["lastVersion"].as<int>();
  totalNode = StatcJson["totalNode"].as<int>();
  totalRepeater = StatcJson["totalRepeater"].as<int>();
  calBatPusat = StatcJson["calBatPusat"].as<float>();
  timeReportPeriodic = StatcJson["timeReportPeriodic"].as<int>();
  timePeriodicSensor = StatcJson["timePeriodicSensor"].as<int>();
  jumlahSensorParsed = StatcJson["jumlahSensor"].as<int>();
  crc32web = StatcJson["crc32web"].as<String>();
  
  // Ambil dan cetak data sensor
  Serial.println("Sensor Data:");
  JsonObject sensor = StatcJson["sensor"];
...
void vPengambilanDataSpreadSheet(){
  ...
  char epochMs[20];
    ...
    arrayJson[36][counterParsing] = USDPSekitar1.toFloat();
    arrayJson[37][counterParsing] = USDPSekitar2.toFloat();
    arrayJson[38][counterParsing] = out_pid1;
    arrayJson[39][counterParsing] = out_pid2;
    arrayJson[40][counterParsing] = out_pid3;
    arrayJson[41][counterParsing] = CounterDataError;
...
void vPengirimanSpreadsheet(String payload) {
  Serial.println("\n--- Mulai Upload Spreadsheet ---");
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("WiFi not connected!");
    return;
  ...
  Serial.print("POST ");
  Serial.print(serverPathSpreadSheet);
  Serial.println(" HTTP/1.1");
  ...
  Serial.print("Host: ");
  Serial.println(serverNameSpreadSheet);
  Serial.println("User-Agent: ESP32");
  Serial.println("Connection: close");
  Serial.println("Content-Type: application/json");
  Serial.print("Content-Length: ");
  Serial.println(TotalPanjangJson);
  Serial.println(); // pemisah header-body
...
bool sendFileFirebase(const char* host,
                    int port,
                    const char* path,
                    const char* filePathOnSd,
                    const char* fileNameOnSd) {
  ...
  if(xSemaphoreTake(sdCardMutex, pdMS_TO_TICKS(50000)) == pdTRUE){
    fileFirebase = SD.open(filePathOnSd, FILE_READ);
    if (!fileFirebase) {
  ...
  if(xSemaphoreTake(sdCardMutex, pdMS_TO_TICKS(50000)) == pdTRUE){
    filename = fileFirebase.name();
    fileLen = fileFirebase.size();
    CRC32File = vCheckCRCfile(SD, filePathOnSd, 20000);
    xSemaphoreGive(sdCardMutex);
  }

  // ==== Persiapan Header, Body, dan Tail ====
  char headTemp[200];
  char headCRCTemp[200];
  char tailTemp[50];
  ...
  uint32_t extraLen = head.length() + headCRC.length() + tail.length();
  uint32_t totalLen = fileLen + extraLen;

  // ==== Kirim request line & header ====
  Serial.println("\n=== [KIRIM REQUEST KE SERVER] ===");
  Serial.print("POST "); Serial.print(serverPath); Serial.println(" HTTP/1.1");
  client.print("POST "); client.print(serverPath); client.println(" HTTP/1.1");

  Serial.print("Host: "); Serial.println(serverName);
  client.print("Host: "); client.println(serverName);

  Serial.print("Content-Length: "); Serial.println(totalLen);
  client.println("Content-Length: " + String(totalLen));
  ...
  // ==== Kirim bagian body ====
  Serial.println(">> [Bagian headCRC]");
  Serial.print(headCRC);
  client.print(headCRC);

  Serial.println(">> [Bagian head]");
  Serial.print(head);
  client.print(head);

  // ==== Kirim isi file (buffer) ====
  Serial.println(">> [Mengirim isi buffer CSV ke server]");
  ...
  if (xSemaphoreTake(sdCardMutex, pdMS_TO_TICKS(50000)) == pdTRUE) {
    FileAda = fileFirebase.available();
    xSemaphoreGive(sdCardMutex);
  }

  while (FileAda) {
    if (xSemaphoreTake(sdCardMutex, pdMS_TO_TICKS(50000)) == pdTRUE) {
      n = fileFirebase.read(buf, bufSize);
      FileAda = fileFirebase.available();
      xSemaphoreGive(sdCardMutex);
    }
    if (n > 0) {
  ...
  Serial.println();
  Serial.print("Selesai kirim buffer. Total byte dikirim: ");

  // ==== Kirim bagian akhir ====
  Serial.println(">> [Bagian tail]");
  Serial.println(tail);
  client.print(tail);
  ...
  client.flush();
  Serial.flush();
  Serial.println("=== [SELESAI KIRIM REQUEST] ===\n");
  ...
  //   client.print("Content-Length: "); client.println(clBuf);
  // }
  ...
  // client.print(headTimestamp);
  // client.print(headSignal);
  // client.print(tail);

  // --- Kirim request line dan headers ---

  // Serial.println("\n=== [KIRIM REQUEST KE SERVER] ===");
  // Serial.print("POST "); Serial.print(path); Serial.println(" HTTP/1.1");
  // client.print("POST "); client.print(path); client.println(" HTTP/1.1");
  ...
  //   char clBuf[32];
  //   snprintf(clBuf, sizeof(clBuf), "%lu", (unsigned long)totalLen);
  //   Serial.print("Content-Length: "); Serial.println(clBuf);
  //   client.print("Content-Length: "); client.println(clBuf);
  // }

  // Serial.print("Content-Type: multipart/form-data; boundary="); Serial.println(boundary);
  // client.print("Content-Type: multipart/form-data; boundary="); client.println(boundary);

  // client.println();
  // Serial.println();

  // // --- Tulis multipart body ---
  // Serial.println(">> [headCRC]");
  // Serial.println(headCRC);
  // client.print(headCRC);

  // Serial.println(">> [headFile]");
  // Serial.println(headFile);
  // client.print(headFile);
  ...
  //     Serial.println("Gagal membuka file SD!");
  //   } else {
  //     file = SD.open(filePathOnSd, FILE_READ);
  //     while (file.available()) {
  //         size_t bytesRead = file.read(buffer, bufferSize);
  //         client.write(buffer, bytesRead);
  //       Serial.write(buffer, bytesRead);
  ...
  //   xSemaphoreGive(sdCardMutex);
  // } else {
  //   Serial.println("Gagal mendapatkan mutex SD Card!");
  // }

  // client.println();
  // Serial.println();

  // // --- Tambahan bagian akhir ---
  // Serial.println(">> [headTimestamp]");
  // Serial.println(headTimestamp);
  // client.print(headTimestamp);

  // Serial.println(">> [headSignal]");
  // Serial.println(headSignal);
  // client.print(headSignal);
  ...
  // Serial.println("=== [SELESAI MENGIRIM REQUEST] ===\n");
  ...
  // Read response (optional)
    ...
    Serial.println("\n=== [RESPON SERVER MENTAH] ===");
    Serial.println(respBuf);


    // --- Ambil bagian JSON dari respon HTTP ---
    char *jsonStart = strchr(respBuf, '{');
    if (!jsonStart) {
      Serial.println("❌ JSON tidak ditemukan dalam respon!");
    }

    // Buat salinan JSON ke buffer baru agar tidak terganggu oleh data lain
    String jsonString = String(jsonStart);

    // Hilangkan karakter non-printable di akhir (misal dari HTTP chunked)
    jsonString.trim();

    Serial.println("\n=== [RESPON JSON UTUH] ===");
    Serial.println(jsonString);
    vParsingResponseWebV2(jsonString.c_str());
    bool statusOK = strstr(respBuf, "200 OK") != nullptr;
    if (statusOK) {
      Serial.println("\n✅ Status HTTP 200 OK - Parsing Firebase...");
      if(xSemaphoreTake(sdCardMutex, pdMS_TO_TICKS(50000)) == pdTRUE){
        deleteFile(SD, filePathOnSd);
        UrutanFirebase++;
        if(UrutanFirebase >= 1440){
          UrutanFirebase = 0;
        }
        preferences.putInt("UrutanFirebase", UrutanFirebase);
        xSemaphoreGive(sdCardMutex);
...
bool sendFileSpreadsheet(const char* host,
                    int port,
                    const char* path,
                    const char* filePathOnSd
                    ) {
  ...
  if(xSemaphoreTake(sdCardMutex, pdMS_TO_TICKS(50000)) == pdTRUE){
    fileSpreadsheet = SD.open(filePathOnSd, FILE_READ);
    if (!fileSpreadsheet) {
      Serial.print("Failed to open SD file: ");
      Serial.println(filePathOnSd);
      fileSpreadsheet.close();
      xSemaphoreGive(sdCardMutex);
      return false;
    }
    xSemaphoreGive(sdCardMutex);
  ...
  // ==== Kirim isi file (buffer) ====
  Serial.println(">> [Mengirim isi buffer JSON ke server]");
  const size_t bufSize = 1024;
  uint8_t buf[bufSize];
  size_t n;
  bool FileAda;
  ...
  if (xSemaphoreTake(sdCardMutex, pdMS_TO_TICKS(50000)) == pdTRUE) {
    FileAda = fileSpreadsheet.available();
    xSemaphoreGive(sdCardMutex);
  }

  while (FileAda) {
    if (xSemaphoreTake(sdCardMutex, pdMS_TO_TICKS(50000)) == pdTRUE) {
      n = fileSpreadsheet.read(buf, bufSize);
      FileAda = fileSpreadsheet.available();
      xSemaphoreGive(sdCardMutex);
    }
    if (n > 0) {
  ...
  // Read response (optional)
    ...
    Serial.println("\n=== [RESPON SERVER MENTAH JSON] ===");
    Serial.println(respBuf);
    bool statusOK = strstr(respBuf, "302 Moved") != nullptr;
    if (statusOK) {
      Serial.println("\n✅ Status HTTP 200 OK - Parsing JSON...");
      if(xSemaphoreTake(sdCardMutex, pdMS_TO_TICKS(50000)) == pdTRUE){
        deleteFile(SD, filePathOnSd);
        UrutanSpread++;
        if(UrutanSpread >= 1440){
          UrutanSpread = 0;
        }
          preferences.putInt("UrutanSpread", UrutanSpread);
        xSemaphoreGive(sdCardMutex);
