...
String data[28];  // Stores data1 to data20
...
String vSendDataHeat() {
  ...
  sprintf(data, "%d,%d,%d,%d,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.2f,%.2f,%.2f", Suhu_Tunnel, Power_Tunnel_1, Heater_st_1, Fan_Power_1, CalM_DinD1, CalB_DinD1, CalM_DinD2, CalB_DinD2, CalM_DinD3, CalB_DinD3, DindSuhu1, DindSuhu2, DindSuhu3);
...
void vAsyncWebServer() {
  ...
  server.serveStatic("/highcharts-more.js", LittleFS, "/highcharts-more.js");
    ...
    server.on("/dataDin", HTTP_GET, [](AsyncWebServerRequest *request){
      ...
      json += "\"BME280\":" + safeValue(String(BME280));
  ...
  server.on("/get", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    if (request->hasParam("In_Heat_st")) {
      ...
      if(Heater_st == 1){
        String Perintah = "KIRIM HEATER Heat_st = 1";
        Serial2.println("<" + Perintah + ">");  // format dengan pembuka & penutup
      }
      else{
        String Perintah = "KIRIM HEATER Heat_st = 0";
        Serial2.println("<" + Perintah + ">");  // format dengan pembuka & penutup
      }
    }
    ...
    if (request->hasParam("In_Fan_Power")) {
      ...
      if(Fan_Power == 1){
        String Perintah = "KIRIM HEATER Fan_Power = 1";
        Serial2.println("<" + Perintah + ">");  // format dengan pembuka & penutup
      }
      else{
        String Perintah = "KIRIM HEATER Fan_Power = 0";
        Serial2.println("<" + Perintah + ">");  // format dengan pembuka & penutup
      }
    }

    if (request->hasParam("In_Power_Tunnel")) {
      Power_Tunnel = request->getParam("In_Power_Tunnel")->value().toInt();
      Serial.print("Power_Tunnel :");
      Serial.println(Power_Tunnel);
      char order[100];
      sprintf(order, "KIRIM HEATER Power_Tunnel = %d", Power_Tunnel);
