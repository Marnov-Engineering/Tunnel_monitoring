...
<head>
  ...
  <script src="SuhuPipaData.js"></script>
  <script src="SuhuDindingData.js"></script>
...
.heater-card {
  flex: 0 0 calc((1200px - 30px) / 3); /* 3 card + gap */
  height: 100%;
  background: #fff;
  padding: 15px;
  border-radius: 8px;
...
<body>
  ...
  <script>
    ...
    const chart1 = Highcharts.chart('chart1', {
      ...
      tooltip: { xDateFormat: '%H:%M:%S', shared: true },
      series: [
        { name: 'Suhu Dalam Pipa 1', data: [] },
        { name: 'Suhu Dalam Pipa 2', data: [] },
        { name: 'Suhu Dalam Pipa 3', data: [] }
      ]
    });
