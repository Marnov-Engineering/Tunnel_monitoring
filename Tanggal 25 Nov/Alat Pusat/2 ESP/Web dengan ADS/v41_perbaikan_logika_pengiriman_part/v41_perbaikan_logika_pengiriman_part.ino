        ...
        if(counterParsing >= 12){
          ...
          if(part > 0){
            ...
            for(int i = 0; i < part; i++){
              ...
              Serial.println(fileLocUpload);
              if (vBacaFileDariSD(fileLocUpload)) {
                vSendFirebasePart(fileLocUpload, fileNameUpload);
              }
            }
          }
  ...
  if (client.connect(serverName, serverPort)) {
    ...
    // if(xSemaphoreTake(sdCardMutex, pdMS_TO_TICKS(50000)) == pdTRUE){
      if(statusCode != 200){
        ...
        if(!PartTidakTerkirim){
          part = 0;
          preferences.putInt("part", part);
        }
        else{
          PartTidakTerkirim = false;
        }
      }
...
void vSendFirebasePart(const char* fileLocUpload , const char* fileNameUpload) {
  if (client.connect(serverName, serverPort)) {
...
void vPengambilanDataSpreadSheet2(){
  Serial.println("Pengambilan data untuk spreadsheet");
  Serial.print("untuk data ke ");
  Serial.println(counterParsing);
  ...
  char epochMs[20];
    ...
    row["Time"] = String(epochMs);
    row["S1"]  = suhu1.toFloat();
    row["S2"]  = suhu2.toFloat();
    row["S3"]  = suhu3.toFloat();
    row["S4"]  = suhu4.toFloat();
    row["S5"]  = suhu5.toFloat();
    row["S6"]  = suhu6.toFloat();
    row["S7"]  = suhu7.toFloat();
    row["SU1"] = Usuhu1.toFloat();
    row["SU2"] = Usuhu2.toFloat();
    row["SD1"] = SuhuDS1;
    row["SD2"] = SuhuDS2;
    row["SD3"] = SuhuDS3;
    row["SP1"] = BME1;
    row["SP2"] = BME2;
    row["SP3"] = BME3;
    row["H1"]  = Humid1.toFloat();
    row["H2"]  = Humid2.toFloat();
    row["H3"]  = Humid3.toFloat();
    row["H4"]  = Humid4.toFloat();
    row["H5"]  = Humid5.toFloat();
    row["H6"]  = Humid6.toFloat();
    row["H7"]  = Humid7.toFloat();
    row["HU1"] = UHumid1.toFloat();
    row["HU2"] = UHumid2.toFloat();
    row["K1"]  = kec1.toFloat();
    row["K2"]  = kec2.toFloat();
    row["K3"]  = kec3.toFloat();
    row["K4"]  = kec4.toFloat();
    ...
    row["K6"]  = kec6.toFloat();
    row["K7"]  = kec7.toFloat();
    row["KU1"] = Ukec1.toFloat();
    row["KU2"] = Ukec2.toFloat();
    row["PU1"] = USDP1.toFloat();
    row["PU2"] = USDP2.toFloat();
    row["PID1"] = out_pid1;
    row["PID2"] = out_pid2;
    row["PID3"] = out_pid3;
    row["Error"] = CounterDataError;
    delay(1);
    Serial.println("Berhasil menambahkan data doc");
}
...
void vPengirimanSpreadsheet() {
  ...
  Serial.println("Selesai memasukan data!");
  counterParsing = 0;
