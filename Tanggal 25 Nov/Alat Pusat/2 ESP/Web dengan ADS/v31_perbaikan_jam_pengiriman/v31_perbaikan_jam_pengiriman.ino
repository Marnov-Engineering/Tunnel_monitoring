...
int sizeofarrayjson = 12;
char BufferSDcard[6000];
...
void vPengambilanDataSdcard(){
  char BufferFirebase[3000];  // perbesar biar cukup
  sprintf(BufferFirebase,
          ...
          // waktuUpdate,SuhuDS3,
          waktuUpdate,BME1 );
  strcat(BufferSDcard, BufferFirebase);
  Serial.println("BufferSDcard tunnel data");
  Serial.println(BufferSDcard);
...
void vTulisKeSDcard(){
  if(isSdCardLogging){
      ...
      // writeBufferSuhu("/SuhuData.js");
      // writeBufferKelembapan("/HumidData.js");
      // writeBufferWind("/WindData.js");
      // writeBufferTekanan("/TekananData.js");
      // writeBufferOutPID("/OutPID.js");
...
void setup() {
  ...
  //   TaskPenulisanSDcard,   /* Task function. */
  //   "TaskPenulisanSDcard",     /* name of task. */
        ...
        if(xSemaphoreTake(sdCardMutex, pdMS_TO_TICKS(50000)) == pdTRUE){
          ...
          if (part > 0){
            ...
            for(int i = 0; i < part; i++){
              ...
              Serial.println(fileLocUploadPart);
              if(xSemaphoreTake(sdCardMutex, pdMS_TO_TICKS(50000)) == pdTRUE){
                if (vBacaFileDariSD(fileLocUploadPart)) {
                  v_SendFilePart(i);
                }
                xSemaphoreGive(sdCardMutex);
              }
...
// void TaskPenulisanSDcard( void * pvParameters ){
//   (void) pvParameters;
//   TickType_t xLastWakeTime;
//   TickType_t xFrequency = 100/ portTICK_PERIOD_MS;
//   xLastWakeTime = xTaskGetTickCount ();
