...
<body>
  ...
  <main>
    ...
    <div class="chart-container"><div id="chart3" class="chart-box"></div></div>
    <div class="chart-container"><div id="chart2" class="chart-box"></div></div>

    <div class="chart-container"><div id="chart4" class="chart-box"></div></div>
    <div class="chart-container"><div id="chart5" class="chart-box"></div></div>
    <div class="chart-container"><div id="chart6" class="chart-box"></div></div>
    ...
    <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 20px; max-width: 800px; margin: 0 auto;">
      ...
      <div class="control-section">
        ...
        <p>Arus = <span id="Arus">0</span> A</p>
  ...
  <footer>
    &copy; 2025 | Sistem Monitoring Suhu ESP32
  </footer>
  ...
  <script>
    ...
    const chart2 = Highcharts.chart('chart2', {
      chart: { type: 'spline' },
      title: { text: 'Grafik Tekanan (SDP810)' },
      xAxis: { type: 'datetime', title: { text: 'Waktu' } },
      yAxis: { title: { text: 'Tekanan' } },
      tooltip: { xDateFormat: '%H:%M:%S', shared: true },
      series: [
        { name: 'Tekanan Pipa 1', data: [] },
        { name: 'Tekanan Pipa 2', data: [] }
      ]
    });

    const chart1 = Highcharts.chart('chart1', {
      chart: { type: 'spline' },
      title: { text: 'Grafik Suhu Tunnel' },
      xAxis: { type: 'datetime', title: { text: 'Waktu' } },
      yAxis: { title: { text: 'Suhu (°C)' } },
      tooltip: { xDateFormat: '%H:%M:%S', shared: true },
      series: [
        { name: 'Suhu Ujung Pipa 1', data: [] },
        { name: 'Suhu Ujung Pipa 2', data: [] },
        { name: 'Suhu 1', data: [] },
        { name: 'Suhu 2', data: [] },
        { name: 'Suhu 3', data: [] },
        { name: 'Suhu 4', data: [] },
    ...
    const chart3 = Highcharts.chart('chart3', {
      chart: { type: 'spline' },
      title: { text: 'Grafik Kecepatan Udara Tunnel' },
      xAxis: { type: 'datetime', title: { text: 'Waktu' } },
      yAxis: { title: { text: 'm/s' } },
      tooltip: { xDateFormat: '%H:%M:%S', shared: true },
      series: [
        { name: 'Kecepatan Ujung Pipa 1', data: [] },
        { name: 'Kecepatan Ujung Pipa 2', data: [] },
        { name: 'Kecepatan 1', data: [] },
        { name: 'Kecepatan 2', data: [] },
        { name: 'Kecepatan 3', data: [] },
        { name: 'Kecepatan 4', data: [] },
        { name: 'Kecepatan 5', data: [] },
        { name: 'Kecepatan 6', data: [] },
        { name: 'Kecepatan 7', data: [] }
      ]
    });
    ...
    for (let i = 1; i < DataAll.length; i++) {
        const row = DataAll[i];
        jarakData.push([row[0], row[1]]);
        load1Data.push([row[0], row[2]]);
        load2Data.push([row[0], row[3]]);
    }

        // Chart 1: Laser (Jarak)
    Highcharts.chart('chart4', {
        chart: { type: 'spline' },
        title: { text: 'Grafik Laser Sensor' },
        xAxis: {
            type: 'linear', // pakai linear agar tidak diformat jadi tanggal
            title: { text: 'Millis (ms)' },
            labels: {
            formatter: function () {
                return this.value; // tampilkan nilai mentah millis
            }
            }
        },
        yAxis: {
    ...
    // Chart 2: Loadcell Vertical
    Highcharts.chart('chart5', {
        chart: { type: 'spline' },
        title: { text: 'Loadcell Vertical' },
        xAxis: {
            type: 'linear', // pakai linear agar tidak diformat jadi tanggal
            title: { text: 'Millis (ms)' },
            labels: {
        ...
        series: [{
            name: 'Load Vertical (kN)',
            data: load1Data,
            color: '#DE0FF4'
        }]
    });

    // Chart 3: Loadcell Horizontal
    Highcharts.chart('chart6', {
        chart: { type: 'spline' },
        title: { text: 'Loadcell Horizontal' },
        xAxis: {
            type: 'linear', // pakai linear agar tidak diformat jadi tanggal
            title: { text: 'Millis (ms)' },
            labels: {
    ...
    async function fetchData() {
      try {
        ...
        chart2.series[0].addPoint([x, data.USDP1], true, chart2.series[0].data.length > 30);
        chart2.series[1].addPoint([x, data.USDP2], true, chart2.series[1].data.length > 30);

        // Jika data.suhu adalah array
        chart1.series[0].addPoint([x, data.Usuhu1], true, chart1.series[0].data.length > 30);
        chart1.series[1].addPoint([x, data.Usuhu2], true, chart1.series[1].data.length > 30);
        chart1.series[2].addPoint([x, data.suhu1], true, chart1.series[2].data.length > 30);
        chart1.series[3].addPoint([x, data.suhu2], true, chart1.series[3].data.length > 30);
        chart1.series[4].addPoint([x, data.suhu3], true, chart1.series[4].data.length > 30);
        chart1.series[5].addPoint([x, data.suhu4], true, chart1.series[5].data.length > 30);
        chart1.series[6].addPoint([x, data.suhu5], true, chart1.series[6].data.length > 30);
        chart1.series[7].addPoint([x, data.suhu6], true, chart1.series[7].data.length > 30);
        chart1.series[8].addPoint([x, data.suhu7], true, chart1.series[8].data.length > 30);

        chart3.series[0].addPoint([x, data.Ukec1], true, chart3.series[0].data.length > 30);
        chart3.series[1].addPoint([x, data.Ukec2], true, chart3.series[1].data.length > 30);
        chart3.series[2].addPoint([x, data.kec1], true, chart3.series[2].data.length > 30);
        chart3.series[3].addPoint([x, data.kec2], true, chart3.series[3].data.length > 30);
        chart3.series[4].addPoint([x, data.kec3], true, chart3.series[4].data.length > 30);
        chart3.series[5].addPoint([x, data.kec4], true, chart3.series[5].data.length > 30);
        chart3.series[6].addPoint([x, data.kec5], true, chart3.series[6].data.length > 30);
        chart3.series[7].addPoint([x, data.kec6], true, chart3.series[7].data.length > 30);
        chart3.series[8].addPoint([x, data.kec7], true, chart3.series[8].data.length > 30);
    ...
    setInterval(function () {
      ...
      xhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
          ...
          document.getElementById("Safety").innerHTML = data[0];
