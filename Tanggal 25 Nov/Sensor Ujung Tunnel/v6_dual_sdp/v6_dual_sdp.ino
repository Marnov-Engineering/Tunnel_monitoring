...
void vKirimData(){
  ...
  String message = String(bme.readTemperature())+ "," + String(bme.readHumidity()) + "," + String(windSpeed1) + "," + String(RealPressure1) + "," + String(RealPressure2);
  // sent = false;
  if (!quickEspNow.send(receiver, (uint8_t *)message.c_str(), message.length()))
  {
      Serial.printf(">>>>>>>>>> Message sent\n");
...
void vSetupSDP(){
  ...
  int ret2 = iInitSensorSPD810(2);
  if (ret2 == 0) {
    Serial.print("iInitSensorSPD810(2): success\n");
  } else {
    Serial.print("iInitSensorSPD810(2): failed, ret = ");
    Serial.println(ret2);
  }
...
void vReadSen1(){
  ...
  if (ret == 0) {
      float DataPascal = mDifferentialPressure;
      double KecepatanRaw = sqrt(2 * DataPascal / 1.225);
      Serial.print("Pa1:");
      Serial.print(DataPascal);
      RealPressure1 = DataPascal;
      // Serial.print(",");
      // Serial.print("Velocity1:");
      // Serial.print(KecepatanRaw);
      // Serial.print(",");
      // Serial.print("Temp: ");
...
void vReadSen2(){
  int ret2 = readSample(2);
  if (ret2 == 0) {
      float DataPascal2 = mDifferentialPressure2;
      double KecepatanRaw2 = sqrt(2 * DataPascal2 / 1.225);
      Serial.print("Pa2:");
      Serial.print(DataPascal2);
      RealPressure2 = DataPascal2;
      // Serial.print(",");
      // Serial.print("Velocity2:");
      // Serial.println(KecepatanRaw2);
      // Serial.print("Temp: ");
