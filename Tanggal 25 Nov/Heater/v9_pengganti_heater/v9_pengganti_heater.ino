...
void dataReceived(uint8_t* address, uint8_t* dataRaw, uint8_t len, signed int rssi, bool broadcast) {
  ...
  // ==== CEK JIKA SCAN ALAT ====
    if (incoming.equals("SCAN ALAT TUNNEL")) {
      Serial.println(">>>>> Perintah SCAN diterima, kirim daftar MAC kembali <<<<<");
      ...
      if (memcmp(mac, target1, 6) == 0) {
        ...
        NamaAlat = "BELUM TERDAFTAR HEATER";
      }
      ...
      macList = String(buf);
      char Balikan[200];
      sprintf(Balikan, "HASIL SCAN ALAT TUNNEL %s MAC %s", NamaAlat.c_str(), macList.c_str());


      if (!quickEspNow.send(address, (uint8_t *)Balikan, strlen(Balikan))) {
        Serial.println("Daftar MAC terkirim ke pengirim scan");
        Serial.println(Balikan);
      } else {
        Serial.println("Gagal kirim daftar MAC");
      }
      return; // stop di sini, jangan parsing ke data[]
    }
