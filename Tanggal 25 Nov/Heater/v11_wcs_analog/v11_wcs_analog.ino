...
static uint8_t macDinding3[] = { 0x2C, 0x3A, 0xE8, 0x14, 0x85, 0x8F }; // heater 3 berfungsi
static uint8_t macDinding2[] = { 0x4C, 0x11, 0xAE, 0x03, 0x64, 0xF1 };
static uint8_t macDinding1[] = { 0x58, 0xBF, 0x25, 0xC2, 0xFE, 0x9F }; // heater 1 tidak berfungsi
...
void loop() {
  ...
  if (millis() - now1S > millis1S) {
    ...
    if(Nilai_WCS > (Nilai_WCS_setup + 400)){
      Raw_WCS = true;
    }
    else{
      Raw_WCS = false;
