...
int heaterID = -1;      // default: belum ditentgukan
uint8_t* sensorDinding = nullptr;

static uint8_t macDinding3[] = { 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA }; // heater 3 berfungsi
static uint8_t macDinding2[] = { 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA };
static uint8_t macDinding1[] = { 0xCC, 0xCC, 0xCC, 0xCC, 0xCC, 0xCC }; // heater 1 tidak berfungsi
// static uint8_t macDinding2[] = { 0xEC, 0xFA, 0xBC, 0x41, 0x6E, 0xDF }; // heater 2
...
static uint8_t AlatPusat[] = { 0x1C, 0x69, 0x20, 0x96, 0xFC, 0x60 };
    ...
    if (incoming.startsWith(perintah)) {
      ...
      if (eqIndex > 0) {
        ...
        if (varName.indexOf("Power_Tunnel") != -1) {
          MaxFanPower = varValue.toInt();
          Serial.print("Power_Tunnel diterima: ");
          Serial.println(MaxFanPower);
          preferences.putInt("MaxFanPower", MaxFanPower);
        }

        if (varName.indexOf("Max_PID_Tunnel") != -1) {
          MaxPid = varValue.toInt();
          Serial.print("Max_PID_Tunnel diterima: ");
          Serial.println(MaxPid);
          preferences.putInt("MaxPid", MaxPid);
          myPID.SetOutputLimits(0, MaxPid);
        }
...
void setup() {
  ...
  MaxFanPower = preferences.getInt("MaxFanPower",100);
  MaxPid = preferences.getInt("MaxPid",50);
  ...
  Nilai_WCS_setup = analogRead(PIN_WCS);
  Serial.print("Nilai WCS Setup = ");
  Serial.println(Nilai_WCS_setup);
...
void vKirimPusat() {
  ...
  String message = String(Heater_st) + "," + 
                   String(Fan_Power) + "," + 
                   String(MaxFanPower) + "," + 
                   String(Raw_WCS) + "," + 
                   String(SuhuDinding) + "," + 
...
void loop() {
  ...
  if (millis() - now1S > millis1S) {
    ...
    // Raw_WCS = true;
    Nilai_WCS = analogRead(PIN_WCS);
    Serial.print("Nilai WCS = ");
    Serial.println(Nilai_WCS);
    if(Nilai_WCS > (Nilai_WCS_setup + 100)){
      Raw_WCS = true;
    }
    else{
      Raw_WCS = false;
